...
def get_user_followers_details(self, user_id, nfollows):
    user_id = self.convert_to_user_id(user_id)
    followers = self.api.get_total_followers(user_id, nfollows)
    return [str(item) for item in followers][::-1] if followers else []
...
def get_user_following_details(self, user_id, nfollows=None):
    user_id = self.convert_to_user_id(user_id)
    following = self.api.get_total_followings(user_id, nfollows)
    return [str(item) for item in following][::-1] if following else []
...
def get_media_likers(self, media_id):
    ...
    if "users" not in self.api.last_json:
        self.logger.info("Media with %s not found." % media_id)
        return []
    return list(map(lambda user: str(user), self.api.last_json["users"]))
...
def get_media_commenters_details(self, media_id):
    ...
    if 'comments' not in self.api.last_json:
        return []
    return [str(item["user"]) for item in self.api.last_json['comments']]
